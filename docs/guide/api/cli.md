...
> `--port` 和 `--daemon` 仅在无子命令直接启动或 `start` 命令时可用。
...
# 流式输出
tpclaw agent run -m "任务内容" --stream

# JSON 格式输出
tpclaw agent run -m "任务内容" --json

# 加载历史上下文
tpclaw agent run -m "继续上次的任务" --history

# 使用 OpenAI 消息格式（支持图片等多模态）
tpclaw agent run --format json -m '{"role":"user","content":[{"type":"text","text":"描述图片"},{"type":"image_url","image_url":{"url":"file:///path/to/image.png"}}]}'
```
...
| `--history` / `-H` | `false` | 加载历史上下文 |
| `--format` | `text` | 输入格式：`text` 或 `json`（OpenAI 消息协议） |
...
```bash
tpclaw agent chat [-a <agent-id>] [-H]

# 选项
-a, --agent string    智能体 ID（默认 main）
-H, --history         加载历史上下文

# 示例
tpclaw agent chat
tpclaw agent chat -a my-agent
tpclaw agent chat -H                    # 加载历史上下文
```
...
-t, --category string   按分类过滤
-l, --loaded            显示已加载的规则链
--json                  JSON 格式输出

# 示例
tpclaw rules list
tpclaw rules list -t agents --json
...
# 选项
-p, --platform string   平台（必填，feishu/dingtalk/wecom）
-a, --account string    账号 ID（必填）
-m, --message string    消息内容（与 --image、--file 至少填一个）
-i, --image string      图片路径
-f, --file string       文件路径

# 示例
# 发送文本消息到飞书
tpclaw message send oc_xxx -p feishu -a my-account -m "Hello World"

# 发送图片到飞书
tpclaw message send oc_xxx -p feishu -a my-account -i /path/to/image.png

# 发送文件到钉钉
tpclaw message send chat-xxx -p dingtalk -a my-account -f /path/to/file.pdf
```
...
```bash
tpclaw message channels [-p <platform>]

# 示例
tpclaw message channels              # 列出所有通道
tpclaw message channels -p feishu    # 查看飞书通道账户详情
```
